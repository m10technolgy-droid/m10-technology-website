...
function deviceIcon(deviceType: string) {
  ...
  if (type.includes("desktop") || type.includes("tower") || type.includes("pc")) return "🖥️";
      ...
      <main className="mx-auto max-w-4xl px-6 py-12 flex-1 w-full">
        ...
        <ul className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          {items?.map((item) => (
            <li
              key={item.id}
              className="rounded-xl border border-zinc-200 bg-white p-5 shadow-sm flex items-start gap-4"
            >
              <span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-lg bg-zinc-50 text-2xl">
